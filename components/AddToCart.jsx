...
const AddToCartButton = ({ productId, quantity = 1, warranty, additionalProducts = [],extendedWarranty, selectedFrequentProducts = [], stockQuantity = 1,special_price, offer_price, price }) => {
  ...
  className={`px-2 py-2 md:px-2 md:py-3  transition duration-300 flex items-center justify-center 
    ${isOutOfStock
      ? 'bg-gray-400 cursor-not-allowed text-red-600'
      : isLoading
      ? 'bg-gray-400 cursor-not-allowed opacity-75'
      : cartSuccess
      ? 'bg-gray-400 text-green-500'
      : 'bg-white text-[#333333] hover:bg-[#a3ca43]'
    }
    active:scale-95 disabled:active:scale-100
    w-1/2 bg-white hover:bg-gray-100 py-3`}
>
  ...
  {isOutOfStock ? (
    ...
    <>
      <span className="sm:inline">Added to Cart</span>
    </>
  ) : (
    <>
