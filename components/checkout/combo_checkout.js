      ...
      {!isDeliverySaved ? (
        ...
        <div className="p-4 space-y-4">
          ...
          <div className="flex items-center space-x-3">
            ...
            />
            <label htmlFor="storePickup" className="text-gray-700 font-medium">
              Store Pickup
            </label>
          </div>
          ...
          <div>
            ...
            {formData.deliveryType === "store" && (
              <div className="text-gray-600 text-sm">
                {
                  fetchedStores.find((s) => s._id === formData.selectedStore)
                    ?.city
                }{" "}
                (
                {
...
export default function Custom_Combo_Checkout() {
  ...
  useEffect(() => {
    ...
    //       setCombo(parsedData.cart.items);
...
const { subtotal, totalDiscount, grandTotal } = useMemo(() => {
  ...
  const handledelete = async (item, type = null) => {
    const confirm = window.confirm(`you want to delete this item ${item.name}`);
    if (!confirm) return;
    ...
    try {
      ...
      const orderRes = await fetch("/api/orders/add", {
        ...
        body: JSON.stringify({
          ...
          delivery_type:
            formData.deliveryType === "store" ? "store_pickup" : "home",
          pickup_store:
            formData.deliveryType === "store"
      ...
      const combodelete = await fetch("/api/custom_combo/all_delete", {
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (combodelete.status === 401) {
        localStorage.removeItem("token");
        router.push("/login");
        return;
      }

      if (combodelete.status === 200) {
        ...
        const proresponse = await fetch(`/api/product/get/${combo[0]._id}`);
        ...
        trackCheckout({
          ...
          product: {
            ...
            image: `${apiUrl}/uploads/products/` + combo[0].images[0],
            qty: combo.length,
            currency: "INR",
          },
        });
        ...
        const order_amount = `₹${Number(orderData.order.order_amount).toFixed(
          2,
        )}`;
        ...
        // const adminItemsHtml = orderData.order.order_item
        //   .map((item) => {
        ...
        //     "https://bea.eygr.in/api/email/send-msg",
        //     {
        //       method: "POST",
        //       headers: {
        //         Authorization:
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="w-full  rounded-lg  pt-0">
        <div
          className="flex flex-col lg:flex-row mr-[20px]"
              ...
              <div className="bg-gray-50 p-4 rounded-lg">
                <div className="grid lg:grid-cols-2 md:grid-cols-2 grid-cols-1 gap-4">
                  <p>
                    ...
                    {useraddress[selectedAddress].firstName}{" "}
                    {useraddress[selectedAddress].lastName}
                  </p>
                  <p>
                    <span className="font-medium">Phone:</span>{" "}
                    {useraddress[selectedAddress].phonenumber}
                  </p>
                  <p>
      ...
      >
        ...
        >
          ...
          <div
            className="w-full lg:w-2/4 p-6 sticky top-6 self-start h-min-full h-max-fit"
            style={{ backgroundColor: "#d3ffaf" }}
          >
            <div className="mt-1" style={{ marginRight: "100px" }}>
              ...
              {combo.some((item) => item.warranty > 0) && (
                <div className="flex justify-between text-gray-800 font-semibold">
                  <span className="text-[#0069c6] hover:text-[#00badb] text-xs sm:text-sm font-medium">
                    Warranty:
                  </span>
              ...
              {combo.some((item) => item.extendedWarranty > 0) && (
                <div className="flex justify-between text-gray-800 font-semibold pt-2 mt-2">
                  ...
                  <span className="text-sm whitespace-nowrap text-base font-semibold text-red-600">
                    ...
                    {Number(
                      combo.reduce(
                        (sum, item) => sum + (item.extendedWarranty || 0),
                        0,
                      ),
                    ).toFixed(2)}
                  </span>
              ...
              {/* <button 
                onClick={handleSubmit} 
                disabled={loading || combo.length === 0 || !isDeliverySaved}
                className={`mt-6 w-full text-white font-semibold py-3 rounded-lg transition ${
                  loading || combo.length === 0 || !isDeliverySaved
                    ? 'bg-gray-400 cursor-not-allowed' 
                    : 'bg-red-500 hover:bg-red-600'
                }`}
              >
      ...
      {showAuthModal && (
        <AuthModal
          ...
          onSuccess={async () => {
            ...
            const res = await fetch("/api/custom_combo/merge", {
              method: "POST",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-sm w-full text-center">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Please wait while we process your payment and order details.
            </p>
          </div>
        </div>
      )}
