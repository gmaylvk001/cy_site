...
export async function PUT(req, { params }) {
  try {
    ...
    // If product not found in Product collection, check Product_all
    if (!product && productData.item_code) {
      const stagedProduct = await Product_all.findOne({
        item_code: productData.item_code,
      });

      if (!stagedProduct) {
        return NextResponse.json(
          { error: "Product not found in Product or Product_all" },
          { status: 404 }
        );
      }

      // Create in Product collection
      product = await Product.create({
        ...stagedProduct.toObject(),
        ...productData,
        _id: undefined, // force new _id
      });

      // Delete from Product_all
      await Product_all.deleteOne({ _id: stagedProduct._id });

      // Update productId for further updates
    ...
    const existingOverviewImages = Array.isArray(productData.overview_image)
      ? productData.overview_image.filter(
    ...
    const filterIds = (productData.filters ?? [])
    ...
    const normalizedBrandCode =
      typeof productData.brand_code === "string"
        ? productData.brand_code.trim()
        : undefined;
    ...
    const updatedProduct = await Product.findByIdAndUpdate(
      productId,
      {
        ...
        category: chain.category_ids, // Parent category
        sub_category: productData.sub_category, // Subcategory
        category_new: chain.category_md5s,
        sub_category_new: chain.sub_category_new,
        sub_category_new_name: chain.sub_category_new_name,
        images: finalImages,
        // overview_image: savedOverviewImages.length > 0
        //   ? savedOverviewImages
        //   : productData.overview_image,
        overview_image:
          finalOverviewImages.length > 0 ? finalOverviewImages : [],
    ...
    if (product_id) {
      if (filterIds.length != 0) {
        ...
        if (
          productData.removedOverviewImages &&
          productData.removedOverviewImages.length > 0
        ) {
        ...
        if (req.files && req.files.overviewImages) {
          ...
          const overviewImagePaths = overviewImageFiles.map(
            (file) => file.filename
          );
    ...
    return NextResponse.json(
      {
        ...
        extend_warranty: updatedProduct.extend_warranty,
      },
      { status: 200 }
    );
  } catch (error) {
